...
try
{
    ...
    List<FoldersResponse>? folders = await GetFolders(client, jsonOptions);
    ...
    List<VideoResponse>? videos = await GetVideos(selectedFolder.URI, client, jsonOptions);
    ...
    await DownloadVideos(videos, int.Parse(size.Item1.TrimEnd('p')), size.Item2, downloadPath, client);
}
catch (Exception e)
{
...
async Task DownloadVideos(List<VideoResponse> videos, int rendition, long totalSize, string downloadPath, HttpClient client)
{
...
async Task<List<VideoResponse>?> GetVideos(string? folderURI, HttpClient client, JsonSerializerOptions jsonOptions)
{
    ...
    List<VideoResponse>? videos = await GetAllDataFromPages<VideoResponse>(videosUrl, client, jsonOptions);
...
(string, long) SelectSize(List<VideoResponse> videos)
{
...
async Task<List<FoldersResponse>?> GetFolders(HttpClient client, JsonSerializerOptions jsonOptions)
{
    ...
    List<FoldersResponse>? folders = await GetAllDataFromPages<FoldersResponse>(foldersUrl, client, jsonOptions);
...
FoldersResponse SelectFolder(List<FoldersResponse> folders)
{
...
async Task<List<T>?> GetAllDataFromPages<T>(string url, HttpClient client, JsonSerializerOptions jsonOptions)
{
    List<T> list = [];

    PaginationResponse<T>? page = await GetPageOfData<T>($"{url}&page=1", client, jsonOptions);
    ...
    while (page.Paging.Next != null)
    {
        ...
        page = await GetPageOfData<T>($"https://api.vimeo.com{page.Paging.Next}", client, jsonOptions);
...
async Task<PaginationResponse<T>?> GetPageOfData<T>(string url, HttpClient client, JsonSerializerOptions jsonOptions)
{
...
int GetMagnitudeOfNumber(int number)
{
